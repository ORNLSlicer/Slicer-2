...
namespace ORNL {
    ...
    void RegionBase::applyMapping(QSharedPointer<Parameterization> parameterization, QVector3D normal_offset)
    {
        Distance max_seg_len = m_sb->setting<Distance>(Constants::ExperimentalSettings::ConformalSlicing::kMaxSegmentLength);
        double max_x = parameterization->getDimensions().x();
        double max_y = parameterization->getDimensions().y();
        Point last;
        for(Path& path : m_paths)
        {
            for(int i = 0; i < path.size(); i++)
            {
                ...
                // Break segments up that are too long
                if(length > max_seg_len)
                {
                    QVector3D vector = (end - start).toQVector3D().normalized();
                    end = start + (vector * max_seg_len());

                    auto newSegment = segment->clone();
                    newSegment->setStart(end);
                    path.insert(i + 1, newSegment);
                }
                // Convert to UV coords
                start.x(start.x() / max_x);
                start.y(start.y() / max_y);
                end.x(end.x() / max_x);
                end.y(end.y() / max_y);

                auto start_uv_values = parameterization->fromUV(MeshTypes::Point_2(start.x(), start.y()));
                auto end_uv_values = parameterization->fromUV(MeshTypes::Point_2(end.x(), end.y()));

                // If the start and end can be mapped, update their points
                if(start_uv_values.first)
                {
                    Point mapped_point = start_uv_values.second;
                    mapped_point += normal_offset;
                    segment->setStart(mapped_point);
                }
                else
                {
                    Point mapped_point = last;
                    mapped_point += normal_offset;
                    segment->setStart(mapped_point);
                }

                if(end_uv_values.first)
                {
                    Point mapped_point = end_uv_values.second;
                    mapped_point += normal_offset;
                    segment->setEnd(mapped_point);
                }
                else
                {
                    Point mapped_point = last;
