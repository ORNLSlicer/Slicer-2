...
namespace ORNL
{
    ...
    QString DMGDMUWriter::writeInitialSetup(Distance minimum_x, Distance minimum_y, Distance maximum_x, Distance maximum_y, int num_layers)
    {
        ...
        if (m_sb->setting< int >(Constants::PrinterSettings::GCode::kEnableStartupCode))
        {
            rv += "G40 G17 G71 G90" % m_newline %
                    ...
                    "T=\"TPE_2\"" % m_newline %
                    "M06" % m_newline %
                    "D1 ;G43 EQUIVALENT" % m_newline %
                    "G90 G54" % m_newline %
                    "G0 X0.5 Y0.5" % m_newline %
                    "G0 Z100." % m_newline;
        }
    ...
    QString DMGDMUWriter::writeBeforePath(RegionType type)
    {
        ...
        {
            if (type == RegionType::kPerimeter && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kPerimeterStart).isEmpty())
            {
                rv += m_sb->setting< QString >(Constants::ProfileSettings::GCode::kPerimeterStart) % m_newline;
            }
            else if (type == RegionType::kInset && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kInsetStart).isEmpty())
            {
                rv += m_sb->setting< QString >(Constants::ProfileSettings::GCode::kInsetStart) % m_newline;
            }
            else if(type == RegionType::kSkeleton && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kSkeletonStart).isEmpty())
            {
                rv += m_sb->setting< QString >(Constants::ProfileSettings::GCode::kSkeletonStart) % m_newline;
            }
            else if (type == RegionType::kSkin && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kSkinStart).isEmpty())
            {
                rv += m_sb->setting< QString >(Constants::ProfileSettings::GCode::kSkinStart) % m_newline;
            }
            else if (type == RegionType::kInfill && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kInfillStart).isEmpty())
            {
                rv += m_sb->setting< QString >(Constants::ProfileSettings::GCode::kInfillStart) % m_newline;
            }
            else if (type == RegionType::kSupport && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kSupportStart).isEmpty())
            {
    ...
    QString DMGDMUWriter::writeLine(const Point& start_point, const Point& target_point, const QSharedPointer<SettingsBase> params)
    {
    ...
    QString DMGDMUWriter::writeArc(const Point &start_point,
    ...
    QString DMGDMUWriter::writeAfterPath(RegionType type)
    {
        ...
        {
            rv += writeExtruderOff();
            if (type == RegionType::kPerimeter && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kPerimeterEnd).isEmpty())
            {
                rv += m_sb->setting< QString >(Constants::ProfileSettings::GCode::kPerimeterEnd) % m_newline;
            }
            else if (type == RegionType::kInset && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kInsetEnd).isEmpty())
            {
                rv += m_sb->setting< QString >(Constants::ProfileSettings::GCode::kInsetEnd) % m_newline;
            }
            else if (type == RegionType::kSkeleton && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kSkeletonEnd).isEmpty())
            {
                rv += m_sb->setting< QString >(Constants::ProfileSettings::GCode::kSkeletonEnd) % m_newline;
            }
            else if (type == RegionType::kSkin && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kSkinEnd).isEmpty())
            {
                rv += m_sb->setting< QString >(Constants::ProfileSettings::GCode::kSkinEnd) % m_newline;
            }
            else if (type == RegionType::kInfill && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kInfillEnd).isEmpty())
            {
                rv += m_sb->setting< QString >(Constants::ProfileSettings::GCode::kInfillEnd) % m_newline;
            }
            else if (type == RegionType::kSupport && !m_sb->setting< QString >(Constants::ProfileSettings::GCode::kSupportEnd).isEmpty())
            {
