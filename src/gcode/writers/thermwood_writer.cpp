...
namespace ORNL
{
    ...
    QString ThermwoodWriter::writeInitialSetup(Distance minimum_x, Distance minimum_y, Distance maximum_x, Distance maximum_y, int num_layers)
    {
        ...
        if(m_sb->setting< int >(Constants::PrinterSettings::GCode::kEnableBoundingBox))
        {
            rv += m_G1 % m_f % QString::number(m_sb->setting< Velocity >(Constants::ProfileSettings::Travel::kSpeed).to(m_meta.m_velocity_unit)) %
                    m_x % QString::number(minimum_x.to(m_meta.m_distance_unit), 'f', 4) % " Y" % QString::number(minimum_y.to(m_meta.m_distance_unit), 'f', 4) % commentSpaceLine("BOUNDING BOX")
    ...
    QString ThermwoodWriter::writeLine(const Point& start_point, const Point& target_point, const QSharedPointer<SettingsBase> params)
    {
    ...
    QString ThermwoodWriter::writeArc(const Point &start_point,
    ...
    QString ThermwoodWriter::writeShutdown()
    {
        ...
        rv += m_sb->setting< QString >(Constants::PrinterSettings::GCode::kEndCode) % m_newline %
              "M83" % commentSpaceLine("RETURN TO CONTROL LOOP") %
                "M02" % commentSpaceLine("SAFETY EOF");
        return rv;
    }
