...
QString SandiaWriter::writeInitialSetup(Distance minimum_x, Distance minimum_y, Distance maximum_x, Distance maximum_y, int num_layers)
{
...
QString SandiaWriter::writeLine(const Point& start_point, const Point& target_point, const QSharedPointer<SettingsBase> params)
{
    Velocity speed = params->setting<Velocity>(Constants::SegmentSettings::kSpeed);
...
QString SandiaWriter::writeArc(const Point &start_point,
                             ...
                             const QSharedPointer<SettingsBase> params)
{
    ...
    rv += m_i % QString::number(Distance(center_point.x() - start_point.x()).to(m_meta.m_distance_unit), 'f', 3) %
          m_j % QString::number(Distance(center_point.y() - start_point.y()).to(m_meta.m_distance_unit), 'f', 3) %
          m_x % QString::number(Distance(end_point.x()).to(m_meta.m_distance_unit), 'f', 3) %
          m_y % QString::number(Distance(end_point.y()).to(m_meta.m_distance_unit), 'f', 3);
...
QString SandiaWriter::writeCoordinates(Point destination)
{
    ...
    rv += m_x % QString::number(Distance(destination.x()).to(m_meta.m_distance_unit), 'f', 3) %
          m_y % QString::number(Distance(destination.y()).to(m_meta.m_distance_unit), 'f', 3);
