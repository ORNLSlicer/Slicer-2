...
namespace ORNL {
AutoOrientation::AutoOrientation(ClosedMesh mesh, Plane slicing_plane) { m_mesh = mesh; }
...
void AutoOrientation::orient() {
    ...
    // Note: a set of faces might not actually be on the mesh, this is why we keep track if a CandidateOrientation is
    // only on the hull Only valid faces to rotate to will be on the convex hull
    ...
    Angle stacking_pitch =
        global_sb->setting<Angle>(Constants::ProfileSettings::SlicingVector::kStackingDirectionPitch);
    Angle stacking_yaw = global_sb->setting<Angle>(Constants::ProfileSettings::SlicingVector::kStackingDirectionYaw);
    Angle stacking_roll = global_sb->setting<Angle>(Constants::ProfileSettings::SlicingVector::kStackingDirectionRoll);
    ...
    settings->setSetting(Constants::ProfileSettings::SpecialModes::kSmoothing,
                         false); // Disable smoothing, we don't need it here
    ...
    CUDA::GPUAutoOrientation::GPUVolumeCalculator* volume_calc =
        new CUDA::GPUAutoOrientation::GPUVolumeCalculator(faces, vertices, threshold());
...
QVector<AutoOrientation::CandidateOrientation> AutoOrientation::getResults() { return m_candidate_orientations; }
...
AutoOrientation::CandidateOrientation AutoOrientation::getRecommendedOrientation() {
    ...
    std::sort(m_candidate_orientations.begin(), m_candidate_orientations.end(),
              [](const CandidateOrientation& lhs, const CandidateOrientation& rhs) {
    ...
    //        // Find max contour
    //        auto max_contour = std::max_element(m_candidate_orientations.begin(), m_candidate_orientations.end(),
    //        [](const CandidateOrientation& lhs,
    ...
    //        // Find min support
    //        auto max_support = std::min_element(m_candidate_orientations.begin(), m_candidate_orientations.end(),
    //        [](const CandidateOrientation& lhs,
    //                                                                                                                 const CandidateOrientation& rhs)
    //        {
    //            return lhs.support_volume < rhs.support_volume;
...
QQuaternion AutoOrientation::GetRotationForOrientation(CandidateOrientation orientation) {
    ...
    // QSharedPointer<SettingsBase> global_sb = GSM->getGlobal();
    // Angle stacking_pitch =
    // global_sb->setting<Angle>(Constants::ProfileSettings::SlicingAngle::kStackingDirectionPitch); Angle stacking_yaw
    // = global_sb->setting<Angle>(Constants::ProfileSettings::SlicingAngle::kStackingDirectionYaw); Angle stacking_roll
    // = global_sb->setting<Angle>(Constants::ProfileSettings::SlicingAngle::kStackingDirectionRoll);
    ...
    // auto angle = MathUtils::signedInternalAngle(Point(settings_build_vector), Point(0, 0, 0),
