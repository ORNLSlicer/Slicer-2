...
namespace ORNL {
    TemplateSaveDialog::TemplateSaveDialog(QWidget* parent): QDialog(parent), m_filename(QDir::homePath() + "/template.s2c") {
        m_convertable_types = QList<QString> { "location", "distance", "voltage", "speed", "accel", "density", "ang_vel", "time", "temperature", "angle", "area"};
    ...
    void TemplateSaveDialog::checkSelection(QTableWidgetItem* item) {
        ...
        if(item->checkState() == Qt::CheckState::Checked)
        {
            if(std::all_of(valuesToCheck.begin(), valuesToCheck.end(), [](bool b){return b;}))
            {
                ...
                if(std::all_of(m_category_check_boxes.begin(), m_category_check_boxes.end(), [](ProgrammaticCheckBox* b){return b->checkState(); }))
        ...
        else
        {
            if(std::all_of(valuesToCheck.begin(), valuesToCheck.end(), [](bool b){return !b;}))
            {
                ...
                if(std::none_of(m_category_check_boxes.begin(), m_category_check_boxes.end(), [](ProgrammaticCheckBox* b){return b->checkState(); }))
    ...
    void TemplateSaveDialog::updateSelection() {
        ...
        if(std::any_of(m_category_check_boxes.begin(), m_category_check_boxes.end(), [](ProgrammaticCheckBox* b) { return !b->checkState(); }) ||
    ...
    void TemplateSaveDialog::setupTable() {
        ...
        while(true)
        {
            for (auto it : j.items()) {
                ...
                QTableWidgetItem* dis = new QTableWidgetItem(QString::fromStdString(it.value()[Constants::Settings::Master::kDisplay]));
                QTableWidgetItem* cat = new QTableWidgetItem(QString::fromStdString(it.value()[Constants::Settings::Master::kMinor]));
                QTableWidgetItem* maj = new QTableWidgetItem(QString::fromStdString(it.value()[Constants::Settings::Master::kMajor]));
...
//        }
            ...
            return QString::number(base_value.to(PM->getMassUnit() / (PM->getDistanceUnit() * PM->getDistanceUnit() * PM->getDistanceUnit())));
        }
