...
namespace ORNL
{
    GCodeRPBFSaver::GCodeRPBFSaver(QString tempLocation, QString path, QString filename, QString text, GcodeMeta meta, double clockAngle, bool offset_enabled, Angle sector_width) :
      m_temp_location(tempLocation), m_path(path), m_filename(filename), m_text(text), m_selected_meta(meta), m_clock_angle(clockAngle), m_is_offset_enabled(offset_enabled), m_sector_width(sector_width)
    {
        //NOP
    }
