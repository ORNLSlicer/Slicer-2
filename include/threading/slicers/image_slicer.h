...
namespace ORNL {
    ...
    class ImageSlicer : public TraditionalAST {
        ...
        private:
            ...
            void createImageStencilVTK(QVector<PolygonListAndColor> geometryAndColor, int layer, double xResolution, double yResolution, int gridWidth, int gridHeight);

            //! \brief Create a stencil image using ITK (2D optimized version of VTK)
            //! \brief Currently, this function doesn't work quite right. For some reason, ITK will not render certain images correctly when creating the stencil.
            //! \brief I need to figure out why and test speed against VTK implementation.
            //! \param geometryAndColor: Vector of polygonlists from cross-sections and associated colors
            //! \param layer: Current layer needed for file output
            //! \param xResolution: X resolution of images
            //! \param yResolution: Y resolution of images
            //! \param gridWidth: width of entire build volume
            //! \param gridHeight: height of entire build volume
            void createImageStencilITK(QVector<PolygonListAndColor> geometryAndColor, int layer, double xResolution, double yResolution, int gridWidth, int gridHeight);
